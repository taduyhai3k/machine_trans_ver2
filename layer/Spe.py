...
class Spe(nn.Module):
    ...
    def forward(self, x, is_encode = True, encoder_out = None, padding_mask = None, look_ahead_mask = None, padding_global_mask = None, inference_mode = False, k_cache = None, v_cache = None):
        out = x
        is_First = False
        if is_encode:
            ...
            for i in range(self.layer):
                if inference_mode:
                    if is_First:
                        out, k_cache[:, i, :, :, :], v_cache[:, i,:, :, :] = self.spe_layer[i](out, is_encode, encoder_out, padding_mask, look_ahead_mask, padding_global_mask, 
                                                            inference_mode, None, None)
                    else:
                        out, k_cache[:, i, :, :, :], v_cache[:, i,:, :, :] = self.spe_layer[i](out, is_encode, encoder_out, padding_mask, look_ahead_mask, padding_global_mask, 
                                                            inference_mode, k_cache[:, i, :, :, :], v_cache[:, i, :, :, :])     
                else:    
                    out,_,_  = self.decoder_layer[i](out, is_encode, encoder_out,padding_mask, look_ahead_mask, padding_global_mask)     
            return out, k_cache, v_cache                                                                     
